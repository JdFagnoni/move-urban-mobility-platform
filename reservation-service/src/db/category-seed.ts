...
export async function loadSeedCategories(): Promise<SeedCategoryRecord[]> {
  const csvPath = resolve(__dirname, "../../data/categories.csv");
  ...
  for (const row of dataRows) {
    ...
    if (!sourceId) {
      throw new Error(`Category seed CSV contains an empty id value: ${csvPath}`);
    }
